...
| Backoffice Admin     | `/admin`              | `data/admin/adminDashboard.ts`                       | `CatalogReviewItem[]`      | NO — name/category derivan de ProductMaster |
| Content Manager      | `/admin/contenido`    | `data/admin/contentManager.ts`                       | `ContentReviewProduct[]`   | NO — name/category derivan de ProductMaster |
...
### 2. Admin data: identidad resuelta, metadata mock

`contentReviewProducts` y `catalogReviewItems` ahora derivan `name`/`category`/`slug` de `ProductMaster`. Los campos admin mock (`editorialStatus`, `completeness`, `hasImage`, readiness, etc.) permanecen en semillas locales sin duplicar nombres de producto.
...
| `STACK-2026-ADMIN-PRODUCTMASTER-CONSUMER-01`               | `apps/web/src/data/admin/adminDashboard.ts` + `contentManager.ts` | ✅ Admin Dashboard + Content Manager name/category derivan de `ProductMaster` |
